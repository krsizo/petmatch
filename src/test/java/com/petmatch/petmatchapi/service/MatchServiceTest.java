...
class MatchServiceTest {
	...
	@Test
	void shouldFilterOutZeroScoreMatches() {
		User user = User.builder()
			.id(1L)
			.email("test@test.com")
			.role(Role.USER)
		...
		PetMatchResponse validMatch = PetMatchResponse.builder()
			.petId(1L)
			.petName("Buddy")
			.score(80)
			.build();

		PetMatchResponse zeroScoreMatch = PetMatchResponse.builder()
			.petId(2L)
			.petName("Mittens")
			.score(0)
			.build();
		...
		when(userPreferenceRepository.findByUser(user)).thenReturn(Optional.of(preference));
		...
		when(petScoringService.calculateMatch(pet1, preference)).thenReturn(validMatch);
		when(petScoringService.calculateMatch(pet2, preference)).thenReturn(zeroScoreMatch);

		List<PetMatchResponse> result = matchService.getMatchesForCurrentUser(10);
